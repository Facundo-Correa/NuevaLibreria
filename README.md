...
Todo lo anterior está disponible desde GitHub, vamos al grano.

### Modificar la sección del Sidebar de un módulo

Entrar a Modules/[Módulo]/Composers/SidebarViewComposer.php y modificar la siguiente linea:

$view->sidebar->group(__('[Nombre de sección]]'), function (SidebarGroup $group)

### Añadir componentes dentro de un módulo "Seccion del sidebar"

Para agregar secciones sin la necesidad de crear mas modelos agregar un item en el mismo archivo del punto anterior.
 $group->addItem(__('Página de Inicio'), function (SidebarItem $item) {
                $item->id = 'inicio';
                $item->icon = config('typicms.inicios.sidebar.icon');
                $item->weight = config('typicms.inicios.sidebar.weight');
                $item->route('admin::index-inicios');
                $item->append('admin::create-inicio');
            });

### Reasignación de rutas en SidebarViewComposer

 $item->append('admin::create-inicio');
 En esta linea se redirige al archivo create.blade.php que se encuentra en inicios/admin
 Se utiliza el atributo ->name() definido en el devido RouterProvider. 

 $item->append('admin::create-booklists-publicidades');
 En esta linea se redirige al archivo create.blade.php que se encuentra en admin/booklists/publicidades
Se utiliza el atributo ->name() definido en el devido RouterProvider.

 Es realmente muy sencillo. Ante cualquier duda preguntarle al rusito <3 que lo creo.

 ### Definicion de las rutas

 Para definir las rutas asignadas anteriormente, entrar en Modulo/Providers/RouteServiceProvider.php

 Route::middleware('admin')->prefix('admin')->name('admin::')->group(function (Router $router) {
            
            $router->get('inicios', [AdminController::class, 'index'])->name('index-inicios')->middleware('can:read inicios');
            
        });

 Creo que no hay mucho que explicar, es muy intuitivo el funcionamiento.

 ### El i18n y su problema locale al cargar una página del BO

Simplemente eliminar el locale de esta definicion:

Antes - const i18n = new VueI18n({ locale: window.TypiCMS.locale, messages });
Despues - const i18n = new VueI18n({ messages });
