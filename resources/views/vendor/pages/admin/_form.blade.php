...
<div class="tab-content">
    ...
    <div class="tab-pane fade show active" id="tab-content">
            ...
            <div class="col-md-6">
                @foreach ($locales as $lang)
                    <div class="mb-3 form-group-translation">
                        <label class="form-label" for="slug[{{ $lang }}]"><span>{{ __('Url') }}</span>
                            ({{ $lang }})</label>
                        <div class="input-group">
                            <span class="input-group-text">{{ $model->present()->parentUri($lang) }}</span>
                            <input class="form-control @if ($errors->has('slug.' . $lang))is-invalid @endif" type="text"
                                name="slug[{{ $lang }}]" id="slug[{{ $lang }}]"
                                value="{{ $model->translate('slug', $lang) }}"
                                data-slug="title[{{ $lang }}]" data-language="{{ $lang }}">
                            <button class="btn btn-outline-secondary btn-slug"
        ...
        {{-- Clase agregada para poder customizar --}}
        {{--{!! BootForm::textarea(__('Body'), 'body')->addClass('ckeditor-full customizable-editor') !!}--}}
        
        <span id="switch-span" class="switch-span-disabled">
            <añadir_libro editor_id="body" path="/libro" switch="switch-span"></añadir_libro>
        </span>
        <textarea id="body" name="body">{{$model->body}}</textarea>
        
        @push('js')
        <script>
            // || CKEDITOR configuracion para evitar QUE ME BORRE TODO LO QUE PONGO Y LO REEMPLACE POR &nbsp, DIOS || //
            CKEDITOR.config.allowedContent=true;
            
            // || Carga de lo guardado en

            let status = -1;
            let editor = CKEDITOR.replace('body');
            let switcher = document.getElementById('switch-span');

            // || Por defecto el span esta escondido
            switcher.style.display = "none";

            // || Comando personalizado para abrir [Agregar Libro]: Utilizable en exposiciones o nuestra editorial
            editor.addCommand('añadirLibro', {
                exec: function(edit){
                    // || Switch del estado de muestreo
            ...
            // || Boton para ejecutar el comando [Agregar Libro]
            editor.ui.addButton('AñadirLibro', {
                label: 'Añadir libros',
                command: 'añadirLibro',
                toolbar: 'insert',
                // || Icono temporal
        ...
        @can('read page_sections')
            @if ($model->id)
                <item-list url-base="/api/pages/{{ $model->id }}/sections"
                    locale="{{ config('typicms.content_locale') }}" fields="id,image_id,page_id,position,status,title"
                    table="page_sections" title="sections" include="image" appends="thumb" :searchable="['title']"
                    :sorting="['position']">

                    <template slot="add-button" v-if="$can('create page_sections')">
                        @include('core::admin._button-create', ['url' => route('admin::create-page_section', $model->id),
                        'module' => 'page_sections'])
                    </template>

                    <template slot="columns" slot-scope="{ sortArray }">
                        <item-list-column-header name="checkbox"
                            v-if="$can('update page_sections')||$can('delete page_sections')"></item-list-column-header>
                        ...
                        <item-list-column-header name="position" sortable :sort-array="sortArray" :label="$t('Position')">
                    ...
                    <template slot="table-row" slot-scope="{ model, checkedModels, loading }">
                        ...
                        <td v-if="$can('update page_sections')">@include('core::admin._button-edit', ['segment' =>
                            'sections', 'module' => 'page_sections'])</td>
                        <td>
