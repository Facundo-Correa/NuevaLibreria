...
@section('content')
<item-list url-base="/api/booklists/promos" locale="{{ config('typicms.content_locale') }}" fields="id,title,position,booklists_sections_id" include="listsection" table="booklists" title="promos" :exportable="true" :searchable="['title']">

    <template slot="add-button" v-if="$can('create booklists')">
        @include('core::admin._button-create', ['url' => route('admin::create-booklists-promos'), 'module' => 'booklists'])
    </template>
    ...
    <template slot="table-row" slot-scope="{ model, checkedModels, loading }">
        ...
        <td v-if="$can('update booklists')">@include('core::admin._button-edit', ['module' => 'promos'])</td>
        <td>
