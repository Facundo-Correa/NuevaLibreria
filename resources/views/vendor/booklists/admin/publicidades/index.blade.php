...
<item-list url-base="/api/booklists/publicidades" locale="{{ config('typicms.content_locale') }}" fields="id,title,image_id,position,booklists_sections_id" include="listsection" table="booklists" title="publicidades" include="image" appends="thumb" :exportable="true" :searchable="['title']">

    <template slot="add-button" v-if="$can('create booklists')">
        @include('core::admin._button-create', ['url' => route('admin::create-booklists-publicidades'), 'module' => 'booklists'])
    </template>
    ...
    <template slot="table-row" slot-scope="{ model, checkedModels, loading }">
        ...
        <td v-if="$can('update booklists')">@include('core::admin._button-edit', ['module' => 'publicidades'])</td>
        <td>
