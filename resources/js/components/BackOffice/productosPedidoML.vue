            ...
            <div class="modal-productos">
                ...
                <div class="content-productos">
                    <div v-for="producto in productos">
                        ...
                        <a target="_blank" :href="getLink(producto.item.id)" style="margin-bottom: 0.3em; padding-left:15px;">{{producto.item.title}}</a>
                    </div>
                </div>
    ...
    methods: {
        openModal() {
            ...
            if (this.data.data.length == 0) {
                ...
                // || Proseguir || //
                this.data.data[0].productos.map((p) => {
                ...
                this.productos.map((producto) => {
                    this.total += producto.quantity * producto.unit_price;
                });
            }
        },
        closeModal() {
        ...
        getLink(itemID) {
            var itemSplice = itemID.split('');
            var ret = 'https://articulo.mercadolibre.com.ar/';
            for(let i = 0; i<itemSplice.length; i++){
                ret += itemSplice[i];
                if(itemSplice[i] == "A") {
                    ret+='-';
                }
